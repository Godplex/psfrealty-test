...
const getPropertyByCounty = (req, res) => {
    ...
    let listHome = parseJsonData.filter((element) => element.geo_county === query.replaceAll('-', ' ') && element.property_type === "RES")

    res.status(200).json(listHome.splice(0, 15))
}

export default getPropertyByCounty;